...
struct morning_server {
    ...
    struct wl_list keyboards;           // 認識されたキーボードのリスト

    struct wl_listener new_input;       // 新しい入力デバイスの接続を検知するlistener
    struct wl_listener new_output;      // 新しい出力デバイスの接続を検知するlistener
    struct wl_list outputs;             // 出力デバイス(モニター)のリスト
};

// キーボードに関するイベントの情報を保持する構造体
struct morning_keyboard {
    struct morning_server *server;      // waylandサーバー
    struct wlr_keyboard *wlr_keyboard;  // キーボードの情報

    struct wl_listener keyboard_input;  // キーボードからの入力を検知するlistener

    struct wl_list link;                // リスト用
};
